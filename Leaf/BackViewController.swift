...
//        
        ...
        homeViewController.navigationItem.leftBarButtonItem?.action = #selector(BackViewController.showLeft)
        homeViewController.navigationItem.rightBarButtonItem?.action = #selector(BackViewController.showRight)
        
        // 给主视图绑定 UIPanGestureRecognizer
        let panGesture = homeViewController.panGesture
        panGesture?.addTarget(self, action: #selector(BackViewController.pan(_:)))
    ...
    func pan(_ recongnizer: UIPanGestureRecognizer) {
        ...
        // 执行平移和缩放动画
        recongnizer.view!.center = CGPoint(x: self.view.center.x + trueDistance*1.2, y: self.view.center.y)
...
//        leftViewController.view.transform = CGAffineTransform.identity.scaledBy(x: pro, y: pro)
    ...
    func doTheAnimate(_ proportion: CGFloat, showWhat: String) {
        UIView.animate(withDuration: 0.3, delay: 0, options: UIViewAnimationOptions(), animations: { () -> Void in
            // 移动首页中心
            self.mainView.center = CGPoint(x: self.view.center.x + self.distance*1.2, y: self.view.center.y)
            // 缩放首页
